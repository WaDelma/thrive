...
public class RadixBalancedTree<T> implements IntMap<T> {
    private Node root;
    private int height;
    private int capacity;
    private int size;

    private RadixBalancedTree(RadixBalancedTree<T> tree){
        root = tree.root;
        height = tree.height;
        size = tree.size;
        capacity = tree.capacity;
    }
    public RadixBalancedTree(){
        root = new RadixNode(new Object[1]);
    }

    @NotNull
    @Override
    public RadixBalancedTree<T> insert(int key, T value) {
        var cur = new RadixBalancedTree<>(this);
    ...
    private RadixBalancedTree<T> append(Object elem) {
        var tree = new RadixBalancedTree<>(this);
        if (needNewRoot()) {
            var ch = new Object[2];
            ch[0] = root;
    ...
    static class RadixNode implements Node {
        ...
        public Node appended(Object elem, int level, int end) {
            if (level == 0) {
                return copyAndUpdate(end & MASK, elem);
            } else {
